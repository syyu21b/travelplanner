...
import {
  ...
  Send, Trash2, TrendingUp, Clock, Users, Star,
  Filter, SortAsc, Plane, BookOpen, Edit2, Check, X, Shield, Copy
} from 'lucide-react';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { StarRatingDisplay } from '@/components/StarRating';
import { TripPlanPreviewDialog } from '@/components/TripPlanPreviewDialog';
...
import type { DiaryEntry, Comment, LinkedPlanPreview } from '@shared/types';
...
export default function Community() {
  ...
  const [linkedPlan, setLinkedPlan] = useState<LinkedPlanPreview | null>(null);
  ...
  // 피드 (서버 페이지네이션)
  const [feedDiaries, setFeedDiaries] = useState<DiaryEntry[]>([]);
  const [totalPages, setTotalPages] = useState(1);
  const [totalCount, setTotalCount] = useState(0);
  const [allTags, setAllTags] = useState<string[]>([]);
  const [stats, setStats] = useState({ totalReviews: 0, destinations: 0, travelers: 0 });
  const [bookmarkedDiaries, setBookmarkedDiaries] = useState<DiaryEntry[]>([]);
  ...
  // 피드 로드 (검색/태그/정렬/페이지 변경 시)
  useEffect(() => {
    let cancelled = false;
    communityApi.listDiaries({ search: debouncedSearch, tag: selectedTag, sort: sortBy, page: currentPage, pageSize: POSTS_PER_PAGE })
      .then(res => {
        if (cancelled) return;
        setFeedDiaries(res.diaries);
        setTotalPages(res.totalPages);
        setTotalCount(res.total);
      })
      .catch(() => { if (!cancelled) toast.error(t('community.admin.errors.storageFull')); });
    return () => { cancelled = true; };
  }, [debouncedSearch, selectedTag, sortBy, currentPage]);

  // 태그/통계는 한 번만 로드
  useEffect(() => {
    communityApi.tags().then(setAllTags).catch(() => {});
    communityApi.stats().then(setStats).catch(() => {});
  }, []);

  // 내가 저장(북마크)한 여행 기록 목록
  useEffect(() => {
    if (!user) { setBookmarkedDiaries([]); return; }
    communityApi.bookmarks().then(setBookmarkedDiaries).catch(() => {});
  }, [user]);

  const openDiaryDetail = (diaryId: string) => {
    communityApi.getDiary(diaryId)
      .then(({ diary, linkedPlan: plan }) => {
        setSelectedDiary(diary);
        setLinkedPlan(plan);
        setActivePhotoIndex(0);
        setShowPlanSchedule(false);
        const draft = loadCommentDraft(diaryId);
        if (draft) setCommentText(draft);
        communityApi.view(diaryId).catch(() => {});
      })
      .catch(() => toast.error(t('community.detail.cloneFailed')));
    communityApi.comments(diaryId).then(setComments).catch(() => setComments([]));
  };
  ...
  // 검색어로 여행지 검색 추적 (현재 로드된 피드 기준의 근사치 — 별도 집계 API를 두지 않은 경량 기능)
  useEffect(() => {
  ...
  // 목록/상세 화면 어디에 있든 동일한 diary id를 즉시 갱신하기 위한 헬퍼
  const patchDiaryEverywhere = (diaryId: string, patch: Partial<DiaryEntry>) => {
    setFeedDiaries(prev => prev.map(d => d.id === diaryId ? { ...d, ...patch } : d));
    setSelectedDiary(prev => prev && prev.id === diaryId ? { ...prev, ...patch } : prev);
  };

  const handleToggleLike = (diaryId: string) => {
    if (!user) { toast.error(t('session.loginRequired')); return; }
    const target = feedDiaries.find(d => d.id === diaryId) || (selectedDiary?.id === diaryId ? selectedDiary : undefined);
    if (!target) return;
    const wasLiked = target.isLikedByMe === true;
    patchDiaryEverywhere(diaryId, { isLikedByMe: !wasLiked, likesCount: target.likesCount + (wasLiked ? -1 : 1) });
    const apiCall = wasLiked ? communityApi.unlike(diaryId) : communityApi.like(diaryId);
    apiCall.catch(() => {
      patchDiaryEverywhere(diaryId, { isLikedByMe: wasLiked, likesCount: target.likesCount });
  ...
  const handleToggleSave = (diaryId: string) => {
    if (!user) { toast.error(t('session.loginRequired')); return; }
    const target = feedDiaries.find(d => d.id === diaryId) || (selectedDiary?.id === diaryId ? selectedDiary : undefined);
    const wasSaved = target?.isBookmarkedByMe === true || bookmarkedDiaries.some(d => d.id === diaryId);
    if (target) patchDiaryEverywhere(diaryId, { isBookmarkedByMe: !wasSaved, bookmarksCount: target.bookmarksCount + (wasSaved ? -1 : 1) });
    setBookmarkedDiaries(prev => wasSaved ? prev.filter(d => d.id !== diaryId) : (target ? [target, ...prev] : prev));
    const apiCall = wasSaved ? communityApi.unbookmark(diaryId) : communityApi.bookmark(diaryId);
    apiCall.then(() => {
      toast.success(wasSaved ? t('community.actions.unsaveToast') : t('community.actions.saveToast'));
    }).catch(() => {
      toast.error(t('community.admin.errors.storageFull'));
    });
  };
  ...
  const handleAddComment = (diaryId: string) => {
    ...
    communityApi.addComment(diaryId, content)
      .then(comment => {
        ...
        const target = feedDiaries.find(d => d.id === diaryId) || (selectedDiary?.id === diaryId ? selectedDiary : undefined);
        if (target) patchDiaryEverywhere(diaryId, { commentsCount: target.commentsCount + 1 });
      })
      .catch(() => toast.error(t('community.admin.errors.storageFull')));
  };
  ...
  const handleDeleteComment = (commentId: string) => {
    ...
    communityApi.deleteComment(commentId)
      .then(() => {
        ...
        if (comment && selectedDiary) patchDiaryEverywhere(comment.diaryId, { commentsCount: Math.max(0, selectedDiary.commentsCount - 1) });
      })
      .catch(() => toast.error(t('community.admin.errors.storageFull')));
  };
  ...
  const handleSaveEditComment = (commentId: string) => {
    ...
    communityApi.updateComment(commentId, content)
      .then(() => {
        setComments(prev => prev.map(c => c.id === commentId ? { ...c, content, updatedAt: new Date().toISOString() } : c));
  ...
  const handleLikeComment = (commentId: string) => {
    ...
    setComments(prev => prev.map(c => c.id !== commentId ? c : {
      ...c,
      likes: wasLiked ? c.likes.filter(id => id !== user.id) : [...c.likes, user.id],
    }));
    const apiCall = wasLiked ? communityApi.unlikeComment(commentId) : communityApi.likeComment(commentId);
    apiCall.catch(() => {
      setComments(prev => prev.map(c => c.id !== commentId ? c : comment));
    });
  };
  ...
  const handleClonePlanToMyPlans = (diary: DiaryEntry) => {
    if (!user) { toast.error(t('session.loginRequired')); return; }
    communityApi.clonePlan(diary.id)
      .then(() => toast.success(t('community.detail.clonedToMyPlans')))
      .catch((err) => {
        if (err instanceof ApiError && err.status === 403) toast.error(t('community.detail.cloneNotAllowed'));
        else if (err instanceof ApiError && err.status === 400) toast.error(t('community.detail.cloneOwnPlan'));
        else toast.error(t('community.detail.cloneStorageFull'));
      });
  };

  // ── 관리자 게시글 삭제 (커뮤니티 공개만 해제, 작성자의 여행 기록 자체는 보존)
  const handleDeleteDiary = (diaryId: string) => {
    diariesApi.setVisibility(diaryId, false)
      .then(() => {
        setFeedDiaries(prev => prev.filter(d => d.id !== diaryId));
  ...
  const handleSaveEditDiary = () => {
    ...
    const patch = {
      title: editDiaryTitle.trim(),
      content: editDiaryContent.trim(),
      location: editDiaryLocation.trim() || editingDiary.location,
    };
    diariesApi.update(editingDiary.id, patch)
      .then(() => {
        patchDiaryEverywhere(editingDiary.id, { ...patch, updatedAt: new Date().toISOString() });
  ...
  // Filter and sort는 이제 서버가 처리 — 페이지에 보이는 목록은 feedDiaries 그대로 사용
  const filteredDiaries = feedDiaries;
  const paginatedDiaries = feedDiaries;
  ...
  if (selectedDiary) {
    const diary = selectedDiary;
    const cmts = comments;
    const isLiked = diary.isLikedByMe === true;
    const isSaved = diary.isBookmarkedByMe === true;
        ...
        <div className="container mx-auto px-4 py-6 max-w-3xl">
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-[#8B7355] flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                {(diary.userName || t('community.userName.traveler')).charAt(0)}
              </div>
              <div>
                <p className="font-bold text-foreground text-sm">{diary.userId === user?.id ? (user?.name || t('community.userName.me')) : (diary.userName || t('community.userName.traveler'))}</p>
          ...
          {diary.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-5">
              {diary.tags.map((tag, i) => (
                <button
                  key={i}
                  onClick={() => { setSelectedDiary(null); setLinkedPlan(null); setShowPlanSchedule(false); setSelectedTag(tag); }}
          ...
          <div className="flex items-center gap-4 py-4 border-t border-b border-border mb-6">
            ...
            >
              ...
              {diary.likesCount > 0 ? diary.likesCount : ''} {t('community.detail.likes')}
            </button>
            <button
            ...
            {bookmarkedDiaries.length > 0 && (
              <Card className="p-4 bg-white border-border">
                <h4 className="font-bold text-foreground text-sm mb-3 flex items-center gap-2">
                  ...
                  {t('community.saved.title', { count: bookmarkedDiaries.length })}
                </h4>
                <div className="space-y-1.5">
                  {bookmarkedDiaries
                    .slice(0, 3)
            ...
            {filteredDiaries.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {paginatedDiaries.map(diary => {
                  const cmtCount = diary.commentsCount;
                  const isLiked = diary.isLikedByMe === true;
                  const isSaved = diary.isBookmarkedByMe === true;
                      ...
                      <div className="p-4 flex flex-col flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-primary to-[#8B7355] flex items-center justify-center text-white font-bold text-[10px] flex-shrink-0">
                            {(diary.userId === user?.id ? (user?.name || '') : (diary.userName || t('community.userName.traveler'))).charAt(0)}
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="text-xs font-bold text-foreground truncate">{diary.userId === user?.id ? (user?.name || t('community.userName.me')) : (diary.userName || t('community.userName.traveler'))}</p>
                        ...
                        <div className="flex items-center gap-1 mt-3 pt-3 border-t border-border">
                          ...
                          >
                            ...
                            {diary.likesCount > 0 && diary.likesCount}
                          </button>
                          <button
                            onClick={() => handleViewDiary(diary)}
                          ...
                          <button
                            onClick={async () => {
                              ...
                              if (user && user.id !== diary.userId) {
                                notificationsApi.share(diary.id).catch(() => {});
                              }
                            }}
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                    <button
                      ...
                      className={cn(
                        ...
                        page === currentPage ? "bg-primary text-white" : "text-muted-foreground hover:bg-secondary"
                      )}
                    >
                      {page}
                    </button>
